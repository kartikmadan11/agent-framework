...
class RawAgent(BaseAgent, Generic[OptionsCoT]):  # type: ignore[misc]
    ...
    async def _prepare_run_context(
        ...
        *,
        ...
        # Auto-inject InMemoryHistoryProvider when session is provided, no history provider
        # registered (even if other context providers exist), and no service-side storage indicators.
        # This ensures multi-turn flows like approval callbacks work correctly even when agents
        # have non-history context providers (e.g. SkillsProvider) but no explicit HistoryProvider.
        if (
            session is not None
